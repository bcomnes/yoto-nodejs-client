 ...
 * @property {string} clientId - Stable unique client ID suffix used to avoid collisions across multiple clients/processes. Used to build the MQTT clientId: DASH${deviceId}${clientId} (non-alphanumeric characters are stripped).
...
export function createYotoMqttClient (options) {
  ...
  const mqttClientId = (`${clientIdPrefix}${deviceId}${clientId}`).replace(/[^a-zA-Z0-9]/g, '')
